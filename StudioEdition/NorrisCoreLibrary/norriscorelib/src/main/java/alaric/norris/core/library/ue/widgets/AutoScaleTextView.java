package alaric.norris.core.library.ue.widgets;
...
public class AutoScaleTextView extends TextView {
    ...
    private void refitText ( String text, int textWidth ) {
        ...
        int targetHeight = this.getHeight() - this.getPaddingTop() - this.getPaddingBottom();
        ...
        while ( ( this.preferredTextSize - this.minTextSize ) > threshold ) {
            ...
            int textHeight = getTextHeight( text, this.getPaint(), targetWidth, size );
            if ( this.textPaint.measureText( text ) >= targetWidth || textHeight >= targetHeight ) {
    ...
    private int getTextHeight (
            CharSequence source, TextPaint originalPaint, int width, float textSize
    ) {
        ...
        StaticLayout layout = new StaticLayout(
                source, paint, width, Alignment.ALIGN_CENTER, 1.0f, 0.0f, true
        );
