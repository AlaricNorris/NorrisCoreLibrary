package alaric.norris.core.library.widgets.passwordwidgets;
...
public class PasswordGestureView extends View {
    ...
    List< String > passwordString = new ArrayList< String >();
    ...
    private void drawCircleAndLines ( Canvas canvas ) {
        ...
        for ( int i = 0 ; i < passwordString.size() ; i++ ) {
            RectF rf = mRectFButtons_select.get( passwordString.get( i ) );
            canvas.drawCircle( rf.centerX(), rf.centerY(), selectButtonCenterR, mPaint );
            if ( isDrawLine ) {

                if ( i > 0 && i < passwordString.size() ) {

                    float x1 = mRectFButtons_select.get( passwordString.get( i - 1 ) ).centerX();
                    float y1 = mRectFButtons_select.get( passwordString.get( i - 1 ) ).centerY();
                    float x2 = mRectFButtons_select.get( passwordString.get( i ) ).centerX();
                    float y2 = mRectFButtons_select.get( passwordString.get( i ) ).centerY();
                    drawlinebyAngle( x1, y1, x2, y2, ArcWidth, canvas, false );

                }
                if ( i == passwordString.size() - 1 && i < mRectFButtons.size() - 1 && ! isError ) {
                    if ( mlastX >= 0 && mlastY >= 0 ) {
                        float x1 = mRectFButtons_select.get( passwordString.get( i ) ).centerX();
                        float y1 = mRectFButtons_select.get( passwordString.get( i ) ).centerY();
    ...
    private void callGetPwd () {
        if ( mGetPwd != null && passwordString.size() > 0 ) {
            String pwd = "";
    ...
    private void drawlinebyAngle (
            float x1, float y1, float x2, float y2, float radius, Canvas canvas, boolean isLast
    ) {
        ...
        if ( isError ) {
            ...
            canvas.drawTextOnPath( "☞", p, getFontHeight( mPaintOrientation, "☞" ) * 3 / 2,
                                   getFontHeight( mPaintOrientation, "☞" ) / 2, mPaintOrientation
            );
        }

        if ( isLast )
            canvas.drawLine( xPoint1, yPoint1, x2, y2, mPaintLine

            );
        else
            canvas.drawLine( xPoint1, yPoint1, xPoint2, yPoint2, mPaintLine
